...
// fetch("products.json")
fetch("script/allcategories.json")
    .then(x => x.json())
    .then(data => {
        for (let index = 0; index < data.allcategories.length; index++) {
            $("#allcategories").append(Products.generateCards(data.allcategories[index]));
